...
const EMPTY_FORM = { title: '', description: '', points_required: 50, repeatable: true }
  ...
  return (
    <div>
      ...
      <form onSubmit={handleCreate} className="bg-slate-800/60 border border-slate-700 rounded-xl p-4 mb-6 flex flex-wrap gap-2">
        ...
        >
          {form.repeatable ? '♾' : '1×'} {form.repeatable ? t.rewards.repeatable : t.rewards.one_time}
        </button>
        <button
          type="submit"
          ...
          return (
            <div
              ...
              className={`relative rounded-xl border p-4 transition-all duration-200
                ${isRedeeming
                  ...
                  : isClaimed
                  ? 'animate-slide-in bg-slate-800/20 border-slate-700/30 opacity-60'
                  : canRedeem
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-3">
                {/* Lock / unlock / claimed icon */}
                <span className="text-lg shrink-0">{isClaimed ? '✅' : canRedeem ? '🔓' : '🔒'}</span>

                <div className="flex-1 min-w-0">
                  <p className={`font-semibold text-sm truncate ${isClaimed ? 'text-slate-600 line-through' : canRedeem ? 'text-white' : 'text-slate-400'}`}>
                    {reward.title}
                  </p>
                  {reward.description && (
                ...
                {isClaimed ? (
                  <span className="shrink-0 text-xs text-slate-600 font-bold">✓ {t.rewards.claimed}</span>
                ) : (
                  <button
              ...
              {/* XP progress bar — hidden for claimed one-time rewards */}
              {!isClaimed && (
                <div className="pl-8">
