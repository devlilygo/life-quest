...
const createReward = (req, res) => {
  ...
  const result = db
    .prepare('INSERT INTO rewards (title, description, points_required) VALUES (?, ?, ?)')
    .run(title, description ?? null, points_required);
...
const updateReward = (req, res) => {
  ...
  db.prepare(`
    ...
    WHERE id = ?
  `).run(title ?? null, description ?? null, points_required ?? null, req.params.id);
